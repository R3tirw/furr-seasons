async function renderStaff() {
  ...
  page.innerHTML = `
    <div class="page-header">
      <div>
        ...
        <div class="page-sub">${staff.length} team member${staff.length !== 1 ? 's' : ''}</div>
      </div>
  ...
  function renderRows(list) {
    ...
    if (!list.length) {
      tbody.innerHTML = '<tr><td colspan="7"><div class="empty-state"><div class="empty-state-icon">👥</div><p>No staff yet</p></div></td></tr>';
      return;
    }
    ...
    return `
      <div class="form-grid">
        <div class="form-group">
          ...
          <input type="text" id="f-name" value="${s.name||''}" placeholder="Rajan Patel" required />
        </div>
        <div class="form-group">
          <label>Role</label>
          <select id="f-role">
            ${['Manager','Caretaker','Dog Trainer','Groomer','Vet Assistant','Receptionist','Other']
              .map(r => `<option value="${r}" ${s.role===r?'selected':''}>${r}</option>`).join('')}
          </select>
        </div>
        <div class="form-group">
          <label>Phone</label>
          <input type="tel" id="f-phone" value="${s.phone||''}" placeholder="+91 98765 43210" />
        </div>
        <div class="form-group">
          <label>Email</label>
        ...
        <div class="form-group span-2">
          <label>Shift</label>
          <select id="f-shift">
            ${['Morning (6am–2pm)','Afternoon (2pm–10pm)','Night (10pm–6am)','Full Day','Flexible']
              .map(sh => `<option value="${sh}" ${s.shift===sh?'selected':''}>${sh}</option>`).join('')}
          </select>
        </div>
      </div>
  ...
  document.getElementById('add-staff-btn').addEventListener('click', () => {
    ...
    document.getElementById('save-staff-btn').addEventListener('click', async () => {
      await api.post('/staff', getFormData());
      closeModal(); showToast('Staff member added ✓'); renderStaff();
    });
  });
  ...
  document.getElementById('staff-body').addEventListener('click', async (e) => {
    ...
    if (editBtn) {
      ...
      document.getElementById('save-staff-btn').addEventListener('click', async () => {
        await api.put('/staff/' + s.id, getFormData());
        closeModal(); showToast('Staff updated ✓'); renderStaff();
      });
    }
    if (delBtn) {
      if (confirm('Remove this staff member?')) {
        await api.delete('/staff/' + delBtn.dataset.id);
        showToast('Staff removed'); renderStaff();
      }
    }
  });
}
